...
DENDRITES_PIPELINE_DIR = Path(__file__).resolve().parents[1]
ROOT_DIR = Path(__file__).resolve().parents[3]
if str(DENDRITES_PIPELINE_DIR) not in sys.path:
    sys.path.insert(0, str(DENDRITES_PIPELINE_DIR))
...
try:
    from . import sleep_dendrite_spine_poster_common as poster_common
except ImportError:
    import sleep_dendrite_spine_poster_common as poster_common

from dendrites_pipeline import (
...
DEFAULT_CACHE_PATH = ROOT_DIR / "results" / "dendrites_pipeline" / "cache" / "sleep_dendrite_spine_cache.npz"
...
def load_cache(path: Path) -> Dict[str, Any]:
    try:
        ...
        json_path = ROOT_DIR / "results" / "dendrites_pipeline" / "analysis_results.json"
        if json_path.exists():
            try:
...
def load_analysis_results_payload() -> Dict[str, Any]:
    ...
    coactivity_base = ROOT_DIR / "results" / "dendrites_pipeline"
...
def write_spine_coactivity_poster_figure(
    cache: Dict[str, Any],
    output_dir: Path,
    *,
    results: Optional[Dict[str, Any]] = None,
    output_stem: str = DEFAULT_SPINE_COACTIVITY_OUTPUT_STEM,
    width_cm: float = DEFAULT_SPINE_COACTIVITY_WIDTH_CM,
    height_cm: float = DEFAULT_SPINE_COACTIVITY_HEIGHT_CM,
) -> Path:
    results = poster_common.load_analysis_results_payload() if results is None else results
    ...
    stem = str(output_stem).strip() or DEFAULT_SPINE_COACTIVITY_OUTPUT_STEM
...
def main() -> int:
    ...
    svg_path = write_spine_coactivity_poster_figure(
        cache,
        results,
        args.output_dir,
